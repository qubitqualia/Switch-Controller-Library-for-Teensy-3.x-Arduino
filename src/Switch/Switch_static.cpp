...
void Switch::handleSwitchPosition(Switch* _switch, int pos) {
	...
	//Handle switch position when it does NOT have ACTIVE Input
		...
		bool pass = false;
		//Check to see if any other switches have control of relay pins
		std::vector<Switch*>::iterator it2;

		for (it2=switch_list.begin(); it2!=switch_list.end(); it2++) {
			for (int i=0;i<10;i++) {
				if (pos == 0) {
					if ((*it2)->pos1->RelayPin[i] == pos_ptr->RelayPin[i]) {
						if (pos_ptr->RelayPin[i] != 255) {
							if ((*it2)->pos1 != pos_ptr) {
								pass = true;
							}
					...
					if ((*it2)->pos2->RelayPin[i] == pos_ptr->RelayPin[i]) {
						if (pos_ptr->RelayPin[i] != 255) {
							if ((*it2)->pos2 != pos_ptr) {
								pass = true;
							}
					...
					if ((*it2)->pos3->RelayPin[i] == pos_ptr->RelayPin[i]) {
						if (pos_ptr->RelayPin[i] != 255) {
							if ((*it2)->pos3 != pos_ptr) {
								pass = true;
							}
