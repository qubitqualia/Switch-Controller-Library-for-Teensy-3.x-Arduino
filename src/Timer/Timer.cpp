...
Stopwatch* Timer::createStopWatch(std::string ctime, bool active) {
...
void Timer::update() {
	...
	for (it=stop_watch_list.begin(); it!=stop_watch_list.end(); it++) {
		...
		//Set alarm time for stopwatch if it hasn't been yet
		if ((*it)->talarm == 0) {
			if ((*it)->clock_time == "") {
...
/*PRIVATE FUNCTIONS*/
void Timer::LocalONtime(Stopwatch* it) {

	getTimeFromString(it->clock_time);
	ton->tm_hour = hh;
	ton->tm_min = mm;
	ton->tm_sec = ss;
	...
	if (mktime(ton) <= current_time) {
		it->interval = 1000*(24*60*60 - (current_time - mktime(ton)));
	}
	else {
		it->interval = 1000*(mktime(ton) - current_time);
	}
}
